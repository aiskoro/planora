...
function Confirmare({ nume, data, ora, oraStop, servicii, durata, onNouaProgramare }) {
  ...
  function descarcaICS() {
    ...
    link.href = url
    link.download = 'programare-timevia.ics'
    link.click()
  ...
  function getLinkGoogleCalendar() {
    ...
    const oraFormatata = ora.replace(':', '') + '00'
    const oraStopFormatata = oraStop.replace(':', '') + '00'
    const titlu = encodeURIComponent('Programare Timevia')
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 20px' }}>
      <style>{`
        ...
        @keyframes fadeUpDelay { 0%, 30% { opacity: 0; transform: translateY(16px); } 100% { opacity: 1; transform: translateY(0); } }
      `}</style>

      <div style={{ maxWidth: '460px', width: '100%', opacity: vizibil ? 1 : 0, transition: 'opacity 0.3s ease' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          <div style={{
            ...
            margin: '0 auto 20px', boxShadow: T.shadowHover,
            animation: 'scaleIn 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)',
          }}>
            <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
              <polyline points="7,18 15,26 29,10" stroke="white" strokeWidth="3"
                strokeLinecap="round" strokeLinejoin="round"
                strokeDasharray="50" strokeDashoffset="0"
                style={{ animation: 'checkDraw 0.4s ease 0.3s both' }} />
            </svg>
          </div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: T.text, margin: '0 0 8px', animation: 'fadeUp 0.4s ease 0.2s both' }}>
            Programare confirmata!
          </h1>
          <p style={{ color: T.muted, fontSize: '15px', margin: 0, animation: 'fadeUp 0.4s ease 0.3s both' }}>
        ...
        <div style={{
          ...
          boxShadow: T.shadowCard, animation: 'fadeUpDelay 0.5s ease both',
        }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {detalii.map((d, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', animation: `fadeUpDelay 0.5s ease ${0.1 * i + 0.4}s both` }}>
                <span style={{ fontSize: '18px', width: '36px', height: '36px', borderRadius: '10px', background: T.accentSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {d.icon}
                </span>
                <div>
        ...
        <p style={{ color: T.muted, fontSize: '13px', textAlign: 'center', marginBottom: '16px', animation: 'fadeUpDelay 0.5s ease 0.6s both' }}>
          Daca ai introdus adresa de email, vei primi o confirmare si acolo.
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', animation: 'fadeUpDelay 0.5s ease 0.7s both' }}>
          <button
          ...
          <button
            ...
            style={{
              padding: '13px 28px', borderRadius: '10px',
              border: `0.5px solid ${hoverIcs ? T.accent : T.border}`,
              backgroundColor: hoverIcs ? T.accentSoft : T.surface,
              color: T.text, fontSize: '14px', fontWeight: '500',
              width: '100%', cursor: 'pointer', transition: T.transition,
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              background: hoverNou ? `linear-gradient(135deg, #5a7af5, ${T.accent})` : `linear-gradient(135deg, ${T.accent}, #3a56d4)`,
              color: '#fff', fontSize: '15px', cursor: 'pointer', fontWeight: '600',
              width: '100%', transition: T.transition,
