...
function Anulare() {
  ...
  if (stare === 'incarcare') return (
    <div style={wrapper}>
      <div style={card}>
        <div style={{ ...iconCircle(T.accentSoft) }}>⏳</div>
        <p style={{ color: T.muted, margin: 0 }}>Se incarca...</p>
      </div>
    </div>
  )
  ...
  if (stare === 'inexistenta') return (
    <div style={wrapper}>
      <div style={card}>
        ...
        <h2 style={{ color: T.text, margin: '0 0 8px', fontSize: '20px', fontWeight: '700' }}>
          Link invalid
        </h2>
        <p style={{ color: T.muted, margin: 0, fontSize: '15px' }}>
          Acest link nu este valid sau a expirat.
        </p>
      </div>
    </div>
  )
  ...
  if (stare === 'expirata') return (
    <div style={wrapper}>
      <div style={card}>
        ...
        <h2 style={{ color: T.text, margin: '0 0 12px', fontSize: '20px', fontWeight: '700' }}>
          Termen depasit
        </h2>
        <p style={{ color: T.muted, margin: '0 0 8px', fontSize: '15px' }}>
          Programarea din <strong style={{ color: T.text }}>{programare.data_programare}</strong> la ora <strong style={{ color: T.text }}>{programare.ora_start.slice(0, 5)}</strong> nu mai poate fi anulata online.
        </p>
        <p style={{ color: T.muted, margin: 0, fontSize: '14px' }}>
          Anularea este posibila doar cu cel putin 2 ore inainte. Te rugam sa ne contactezi direct.
        </p>
      </div>
    </div>
  )
  ...
  if (stare === 'anulata') return (
    <div style={wrapper}>
      ...
      <div style={card}>
        ...
        <p style={{ color: T.muted, margin: '0 0 24px', fontSize: '15px' }}>
          Programarea din <strong style={{ color: T.text }}>{programare.data_programare}</strong> la ora <strong style={{ color: T.text }}>{programare.ora_start.slice(0, 5)}</strong> a fost anulata cu succes.
        </p>
        <button
      ...
      <div style={{ ...card, animation: 'fadeUp 0.3s ease' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
          <button
            ...
            disabled={loading}
            onMouseEnter={() => setHoverAnuleaza(true)}
            onMouseLeave={() => setHoverAnuleaza(false)}
            style={{
              padding: '12px 24px',
              borderRadius: '10px',
              border: 'none',
              background: hoverAnuleaza ? '#dc2626' : T.danger,
              color: '#fff',
              fontSize: '14px',
              fontWeight: '600',
              cursor: loading ? 'wait' : 'pointer',
              transition: T.transition,
              transform: hoverAnuleaza ? 'scale(1.03)' : 'scale(1)',
              boxShadow: hoverAnuleaza ? '0 4px 16px rgba(239,68,68,0.3)' : 'none',
            }}
          >
          ...
          <button
            onClick={() => window.location.href = '/'}
            onMouseEnter={() => setHoverPastreaza(true)}
            onMouseLeave={() => setHoverPastreaza(false)}
            style={{
              padding: '12px 24px',
              borderRadius: '10px',
              border: `0.5px solid ${hoverPastreaza ? T.accent : T.border}`,
              background: hoverPastreaza ? T.accentSoft : T.surface,
              color: hoverPastreaza ? T.accent : T.muted,
              fontSize: '14px',
              fontWeight: '500',
              cursor: 'pointer',
              transition: T.transition,
