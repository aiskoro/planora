...
function ServiciiList({ selectate, onChange, frizerId }) {
  ...
  function toggleServiciu(serviciu) {
    ...
    if (exista) onChange(selectate.filter(s => s.id !== serviciu.id))
    else onChange([...selectate, serviciu])
  }
  ...
  return (
    <div style={{ background: T.surface, border: `0.5px solid ${T.border}`, borderRadius: '16px', padding: '20px', marginBottom: '12px', boxShadow: T.shadowCard }}>
      <style>{`@keyframes pop { 0% { transform: scale(1); } 40% { transform: scale(1.13); } 100% { transform: scale(1); } }`}</style>

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <span style={{ fontSize: '11px', letterSpacing: '0.1em', color: T.muted, textTransform: 'uppercase' }}>Servicii</span>
        {selectate.length > 0 && (
          <span style={{ padding: '3px 10px', borderRadius: '20px', background: T.accentSoft, border: `0.5px solid ${T.accent}`, color: T.accent, fontSize: '12px', fontWeight: '500' }}>
            {durataTotala} min
          </span>
        )}
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                fontSize: '13px', cursor: 'pointer', transition: T.transition,
