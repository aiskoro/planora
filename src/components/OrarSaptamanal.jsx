...
function OrarSaptamanal() {
  ...
  const fetchOrar = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('orar').select('*').order('zi_saptamana')
    setOrar(data || [])
    setLoading(false)
  }, [])
    ...
    <div>
        ...
        {orar.map(zi => (
          <div
            ...
            style={{
              padding: '14px 16px',
              borderRadius: '12px',
              border: `0.5px solid ${zi.deschis ? T.border : 'transparent'}`,
              background: zi.deschis ? T.surface2 : 'rgba(107,114,128,0.06)',
              display: 'flex',
              alignItems: 'center',
              gap: '16px',
              flexWrap: 'wrap',
      ...
      <button
        ...
        style={{
          ...
          transition: T.transition,
          boxShadow: T.shadow,
          transform: hoverSalveaza && !salvat ? 'scale(1.02)' : 'scale(1)',
        }}
      >
