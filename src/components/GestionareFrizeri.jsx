...
function GestionareFrizeri({ isMaster, tenantId }) {
  const { T } = useTheme()
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('tenant_id', tenantId)
      .order('ordine')
      .then(({ data }) => setToateServiciile(data || []))
  }, [fetchAngajati, tenantId])

  async function deschideServicii(angajatId) {
    if (serviciiDeschis === angajatId) { setServiciiDeschis(null); return }
    const { data } = await supabase.from('frizer_servicii').select('serviciu_id').eq('frizer_id', angajatId)
    const asignate = new Set((data || []).map(r => r.serviciu_id))
    setServiciiAsignate(prev => ({ ...prev, [angajatId]: asignate }))
    setServiciiDeschis(angajatId)
  }

  async function toggleServiciu(angajatId, serviciuId) {
    const asignate = serviciiAsignate[angajatId] || new Set()
    if (asignate.has(serviciuId)) {
      await supabase.from('frizer_servicii').delete().eq('frizer_id', angajatId).eq('serviciu_id', serviciuId)
      const nou = new Set(asignate); nou.delete(serviciuId)
      setServiciiAsignate(prev => ({ ...prev, [angajatId]: nou }))
    } else {
      await supabase.from('frizer_servicii').insert({ frizer_id: angajatId, serviciu_id: serviciuId })
  ...
  async function adaugaAngajat() {
    ...
    setSuccess(`Angajatul ${numeNou.trim()} a fost adaugat cu succes!`)
  ...
  async function toggleActiv(angajat) {
    await supabase.from('frizeri').update({ activ: !angajat.activ }).eq('id', angajat.id)
    fetchAngajati()
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
        {angajati.map(a => (
          <div key={a.id}>
            <div style={{ padding: '16px 20px', borderRadius: serviciiDeschis === a.id ? '12px 12px 0 0' : '12px', border: `0.5px solid ${a.activ ? T.border : 'transparent'}`, background: a.activ ? T.surface2 : 'rgba(107,114,128,0.06)', display: 'flex', alignItems: 'center', gap: '14px', opacity: a.activ ? 1 : 0.6, transition: T.transition }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontWeight: '600', fontSize: '15px', color: T.text }}>{a.nume}</span>
                  {a.is_master && <span style={{ fontSize: '11px', color: T.accent, background: T.accentSoft, padding: '2px 8px', borderRadius: '20px', border: `0.5px solid ${T.border}` }}>Master</span>}
                  {!a.activ && <span style={{ fontSize: '11px', color: T.muted, background: T.surface, padding: '2px 8px', borderRadius: '20px', border: `0.5px solid ${T.border}` }}>Inactiv</span>}
                </div>
                <span style={{ fontSize: '13px', color: T.muted }}>{a.email}</span>
              </div>
              ...
              {confirmSterge === a.id ? (
                ...
                <>
                  <button onClick={() => deschideServicii(a.id)} style={{ padding: '6px 12px', borderRadius: '8px', border: `0.5px solid ${serviciiDeschis === a.id ? T.accent : T.border}`, background: serviciiDeschis === a.id ? T.accentSoft : T.surface, color: serviciiDeschis === a.id ? T.accent : T.muted, cursor: 'pointer', fontSize: '13px', fontWeight: '500', whiteSpace: 'nowrap' }}>
                    {serviciiDeschis === a.id ? 'Ascunde' : 'Servicii'}
                  </button>
                  {!a.is_master && (
                    <button onClick={() => toggleActiv(a)} style={{ padding: '6px 12px', borderRadius: '8px', border: `0.5px solid ${a.activ ? 'rgba(239,68,68,0.3)' : 'rgba(34,197,94,0.3)'}`, background: a.activ ? T.dangerSoft : 'rgba(34,197,94,0.08)', color: a.activ ? T.danger : T.success, cursor: 'pointer', fontSize: '13px', fontWeight: '500', whiteSpace: 'nowrap' }}>
                      {a.activ ? 'Dezactiveaza' : 'Activeaza'}
                    </button>
                  )}
                  {isMaster && !a.is_master && (
                    <button onClick={() => setConfirmSterge(a.id)} style={{ padding: '6px 12px', borderRadius: '8px', border: '0.5px solid rgba(239,68,68,0.3)', background: T.dangerSoft, color: T.danger, cursor: 'pointer', fontSize: '13px', fontWeight: '500', whiteSpace: 'nowrap' }}>Sterge</button>
                  )}
                </>
              )}
            </div>
              ...
              <div style={{ padding: '16px 20px', borderRadius: '0 0 12px 12px', border: `0.5px solid ${T.border}`, borderTop: 'none', background: T.surface }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {toateServiciile.map(s => {
                    const bifat = (serviciiAsignate[a.id] || new Set()).has(s.id)
                    return (
                      <label key={s.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', borderRadius: '10px', border: `0.5px solid ${bifat ? T.accent : T.border}`, background: bifat ? T.accentSoft : T.surface2, cursor: 'pointer', transition: T.transition }}>
                        <input type="checkbox" checked={bifat} onChange={() => toggleServiciu(a.id, s.id)} style={{ width: '16px', height: '16px', accentColor: T.accent, cursor: 'pointer' }} />
      ...
      {isMaster && (
        adaugaMode ? (
          <div style={{ padding: '20px', borderRadius: '12px', border: `0.5px solid ${T.border}`, background: T.surface2 }}>
            <span style={{ fontSize: '11px', letterSpacing: '0.1em', color: T.muted, textTransform: 'uppercase', display: 'block', marginBottom: '14px' }}>Angajat nou</span>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <input type="text" placeholder="Nume angajat" value={numeNou} onChange={e => { setNumeNou(e.target.value); setEroare(null) }} style={{ padding: '10px 14px', borderRadius: '10px', border: `0.5px solid ${T.border}`, background: T.surface, color: T.text, fontSize: '14px', outline: 'none' }} />
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                <button onClick={adaugaAngajat} disabled={loadingAdauga} style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', background: `linear-gradient(135deg, ${T.accent}, #3a56d4)`, color: '#fff', fontSize: '14px', fontWeight: '600', cursor: loadingAdauga ? 'wait' : 'pointer', boxShadow: T.shadow }}>
                  {loadingAdauga ? 'Se creeaza...' : 'Adauga angajat'}
                </button>
